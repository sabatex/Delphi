...
{                     EhLib vX.X                        }
{                                                       }
{            TMemTableEh component (Build 15)           }
{                                                       }
...
{ TCustomMemTableEh }
  ...
  TRecordsViewTreeNodeExpandingEventEh = procedure (Sender: TObject; Node: TMemoryTreeNodeEh;
    var AllowExpansion: Boolean) of object;

  TRecordsViewTreeNodeExpandedEventEh = procedure (Sender: TObject; Node: TMemoryTreeNodeEh) of object;

  TRecordsViewCheckMoveNodeEventEh = function (Sender: TObject;
    SourceNode, AppointedParent: TMemoryTreeNodeEh; AppointedIndex: Integer): Boolean of object;
...
//    FOrderByList: TList;
    ...
    FExternalData: TCustomMemTableEh;
...
{$ENDIF}
    ...
    procedure SetExternalData(Value: TCustomMemTableEh);
...
{$ENDIF}
    ...
    function TreeViewNodeExpanding(Sender: TTreeNodeEh): Boolean;
    procedure TreeViewNodeExpanded(Sender: TTreeNodeEh);
...
{$ENDIF}
    ...
    function SetTempRecBufForRecord(Rec: TMemoryRecordEh; TreeNode: TMemoryTreeNodeEh; RecNum: Integer): TRecBuf;
...
//    property OnFetchRecord: TMTFetchRecordEventEh read FOnFetchRecord write FOnFetchRecord;
    ...
    property ExternalData: TCustomMemTableEh read FExternalData write SetExternalData;
  end;
...
{ TRecBuf }
function TRecBuf.GetTreeNode: TMemoryTreeNodeEh;
begin
  Result := nil;
...
begin
  ...
  begin
    ...
    if DataDriver <> nil then
    begin
      //? ƒобавить (FieldCount > 0) then но только не дл€ запроса списка полей в DesignTime.
...
begin
  ...
  if (DataDriver = nil) {and (ProviderDataSet = nil)} and not FRecordsView.MemTableData.IsEmpty then
...
begin
  ...
  begin
    if (Value <> nil) and (ExternalData <> nil) then
      raise Exception.Create('Assigning to DataDriver is not allowed if ExternalData is assigned');
    FDataDriver := Value;
...
begin
  ...
  if GetActiveRecBuf(RecBuf) and (RecBuf.TreeNode <> nil) then
    Result := RecBuf.TreeNode.HasChildren;
end;
...
begin
  ...
  if RecordsView.ViewAsTreeList and (RecNo > 1) then
    Result := RecordsView.MemoryTreeList.VisibleItem[RecNo-2].Level;
end;
...
begin
  ...
  if RecNo < RecordCount then
    Result := RecordsView.MemoryTreeList.VisibleItem[RecNo].Level;
end;
...
begin
  ...
  if TreeNode = nil then
    Exit;
  CurNode := TreeNode;
  while ParenLevel < TreeNode.Level do
  begin
...
function TCustomMemTableEh.SetTempRecBufForRecord(Rec: TMemoryRecordEh;
  TreeNode: TMemoryTreeNodeEh; RecNum: Integer): TRecBuf;
var
{$IFDEF CIL}
...
var
  RecBuf: TRecBuf;
  TreeNode, ActiveTreeNode: TMemoryTreeNodeEh;
//  ARecNo: Integer;
  FindedRecPos: Integer;
  ActiveHided: Boolean;
...
//    OnTreeNodeExpanding(Self, ARecNo, AllowExpansion);
  ...
  if AllowExpansion
    then TreeNode.Expanded := Value
    else Exit;

  RecordsView.MemoryTreeList.BuildVisibleItems;
  ActiveHided := False;
  if ActiveTreeNode <> nil then
    while (FRecordsView.IndexOf(ActiveTreeNode.Data) = -1) and (ActiveTreeNode.Level > 1) do
    begin
      ActiveTreeNode := TMemoryTreeNodeEh(ActiveTreeNode.Parent);
      ActiveHided := True;
    end;
  if ActiveHided then
  begin
    FindedRecPos := FRecordsView.IndexOf(ActiveTreeNode.Data);
    if FindedRecPos <> -1 then
...
begin
  ...
  if GetActiveRecBuf(RecBuf) then //??? Logicaly deen to set IsForWrite = True 
    ...
    begin
      RecBuf.TreeNode.HasChildren := Value;
      Resync([]);
    end else
...
begin
  ...
  Result := False;
  if GetActiveRecBuf(RecBuf) and (RecBuf.TreeNode <> nil) then
    Result := RecBuf.TreeNode.Expanded;
end;
  ...
  if GetActiveRecBuf(RecBuf) then //??? Logicaly need to set IsForWrite = True
    ...
    begin
      ...
      if AllowExpansion then
        RecBuf.TreeNode.Expanded := Value;
      Resync([]);
    end else
      RecBuf.NewTreeNodeExpanded := Value;
end;

function TCustomMemTableEh.TreeViewNodeExpanding(Sender: TTreeNodeEh): Boolean;
var
  RecBuf: TRecBuf;
  ActiveTreeNode: TMemoryTreeNodeEh;
//  AllowExpansion: Boolean;
  ARecNo: Integer;
  MemSender: TMemoryTreeNodeEh;
begin
  MemSender := TMemoryTreeNodeEh(Sender);
  Result := True;
  if not Sender.Expanded and Assigned(OnRecordsViewTreeNodeExpanding) then
    OnRecordsViewTreeNodeExpanding(Self, MemSender, Result);
  if Active and not Sender.Expanded and Assigned(OnTreeNodeExpanding) then
  begin
    if GetActiveRecBuf(RecBuf) and (RecBuf.TreeNode <> nil)
      then ActiveTreeNode := RecBuf.TreeNode
      else ActiveTreeNode := nil;
...
procedure TCustomMemTableEh.TreeViewNodeExpanded(Sender: TTreeNodeEh);
begin
  if Active then
  begin
    RecordsView.MemoryTreeList.BuildVisibleItems;
...
function TCustomMemTableEh.CompareTreeNodes(Rec1, Rec2: TTreeNodeEh; ParamSort: TObject): Integer;
begin
  Result := CompareRecords(TMemoryTreeNodeEh(Rec1).Data, TMemoryTreeNodeEh(Rec2).Data, ParamSort);
end;
...
begin
  ...
  Result := -1;
  if GetActiveRecBuf(RecBuf) and (RecBuf.TreeNode <> nil) then
    Result := RecBuf.TreeNode.VisibleCount;
end;

function TCustomMemTableEh.GetTreeNode: TMemoryTreeNodeEh;
var
  RecBuf: TRecBuf;
begin
  CheckActive;
...
//        FInstantReadMode := False;
    ...
    begin
      ...
      begin
        Result := VarArrayCreate([0, Fields.Count - 1], varVariant);
        for I := 0 to AOrderByList.Count - 1 do
          Result[I] := Rec.Value[AOrderByList[I].FieldNo-1, dvvValueEh];
      end else
        Result := Rec.Value[AOrderByList[0].FieldNo-1, dvvValueEh];
    end;
  end;
...
//      BufferToRecBuf(FInstantBuffer).MemRec := nil;
  ...
  begin
    ...
    begin
      RecordToBuffer(FRecordsView.ViewRecord[RowNum], dvvValueEh, InstantBuffer);
...
procedure TCustomMemTableEh.SetExternalData(Value: TCustomMemTableEh);
var
  WasActive: Boolean;
begin
  ...
  begin
    if (Value <> nil) and (DataDriver <> nil) then
      raise Exception.Create('Assigning to ExternalData is not allowed if DataDriver is assigned');
    WasActive := Active;
...
var
  ...
  FromNode, PrevNode, NextNode: TMemoryTreeNodeEh;
  RefParentValue: Variant;
  NewPos: Integer;

  function InsertAfter(FromNode, AfterNode: TMemoryTreeNodeEh): Boolean;
  var
    IndexInParentNode: Integer;
  begin
    Result := True;
    RefParentValue := AfterNode.Data.DataValues[TreeList.RefParentFieldName, dvvValueEh];
    if AfterNode.Data.Index > FromNode.Data.Index
      then IndexInParentNode := AfterNode.Data.Index
      else IndexInParentNode := AfterNode.Data.Index + 1;

    if Assigned(FOnRecordsViewCheckMoveNode) then
      Result := FOnRecordsViewCheckMoveNode(Self, FromNode, AfterNode.Parent, IndexInParentNode);
    if not Result then Exit;

    if CheckOnly then
      Result := not RecordsView.MemoryTreeList.CheckReferenceLoop(FromNode.Data, RefParentValue)
    else
    begin
      FromRec := FromNode.Data;
      FromRec.Edit;
      FromRec.DataValues[TreeList.RefParentFieldName, dvvValueEh] := RefParentValue;
      FromRec.Post;

      if AfterNode.Data.Index > FromNode.Data.Index
        then RecordsView.MemTableData.RecordsList.Move(FromRec.Index, AfterNode.Data.Index)
        else RecordsView.MemTableData.RecordsList.Move(FromRec.Index, AfterNode.Data.Index+1);
    end;
  end;

  function InsertBefore(FromNode, BeforeNode: TMemoryTreeNodeEh): Boolean;
  var
    IndexInParentNode: Integer;
  begin
    RefParentValue := BeforeNode.Data.DataValues[TreeList.RefParentFieldName, dvvValueEh];
    Result := True;

    if BeforeNode.Data.Index > FromNode.Data.Index
      then IndexInParentNode := BeforeNode.Data.Index - 1
      else IndexInParentNode := BeforeNode.Data.Index;

    if Assigned(FOnRecordsViewCheckMoveNode) then
      Result := FOnRecordsViewCheckMoveNode(Self, FromNode, BeforeNode.Parent, IndexInParentNode);
    if not Result then Exit;
    ...
    begin
      ...
      if BeforeNode.Data.Index > FromNode.Data.Index
        then RecordsView.MemTableData.RecordsList.Move(FromRec.Index, BeforeNode.Data.Index-1)
        else RecordsView.MemTableData.RecordsList.Move(FromRec.Index, BeforeNode.Data.Index);
    end;
  end;

  function InsertChild(FromNode, ParentNode: TMemoryTreeNodeEh): Boolean;
  begin
    Result := True;
    RefParentValue := ParentNode.Data.DataValues[TreeList.KeyFieldName, dvvValueEh];
 ...
 if TreeList.Active then
  begin
    ...
    try
      ...
      FromRec := RecordsView.MemoryTreeList[FromIndex].Data;
      ...
      if (PrevNode <> nil) and (TreeLevel = PrevNode.Level) then
        Result := Result and InsertAfter(FromNode, PrevNode)
      else if (NextNode <> nil) and (TreeLevel = NextNode.Level) then
        Result := Result and InsertBefore(FromNode, NextNode)
      else if (PrevNode <> nil) and (TreeLevel > PrevNode.Level) then
        Result := Result and InsertChild(FromNode, PrevNode)
      else if (PrevNode <> nil) and (TreeLevel < PrevNode.Level) then
      begin
        while PrevNode.Level > TreeLevel do
        begin
          if PrevNode.Parent = PrevNode.Owner.Root then
            Exit;
...
//  if (RecordsList = nil) or (RecordsList.DataStruct.FindField(FieldName) = nil) then
  if (MemTableData = nil) or (MemTableData.DataStruct.FindField(FieldName) = nil) then
    Exit;
