...
type
  ...
  public
    ...
    procedure RecordsViewTreeNodeExpanding(Sender: TObject; Node: TMemoryTreeNodeEh; var AllowExpansion: Boolean);
  end;
...
procedure TForm1.RecordsViewTreeNodeExpanding(Sender: TObject;
  Node: TMemoryTreeNodeEh; var AllowExpansion: Boolean);
var
  i: Integer;
begin
  MemTableEh1.DisableControls;
  if Node.Data.DataValues['SubcLoaded', dvvValueEh] = True then Exit;
  try
    Roots.Clear;
    for i := 0 to Node.Count-1 do
      Roots.AddObject(VarToStr(Node[i].Data.DataValues['FileDirPath', dvvValueEh]), TObject(Integer(Node[i].Data.DataValues['ID', dvvValueEh])) );

    for i := 0 to Roots.Count-1 do
      AddDir(Roots[i], Integer(Roots.Objects[i]));

    Node.Data.DataValues['SubcLoaded', dvvValueEh] := True;
  finally
    MemTableEh1.EnableControls;
  end;
end;
