...
#include "firms.h"
#include "FGInt.hpp"
#include "FGIntPrimeGeneration.hpp"
...
//---------------------------------------------------------------------------
void __fastcall TdmMain::aNewFirmExecute(TObject *Sender)
{
  using namespace :: ntics_forms;
  dmBase->FIRMS->Insert();
  frmFirmEdit = static_cast<TfrmFirmEdit*>(ShowForm(__classid(TfrmFirmEdit),"Ввод новой фирмы",ftModal));
  if (frmFirmEdit->ModalResult == mrOk)
    dmBase->FIRMS->Post();
  else
    dmBase->FIRMS->Cancel();
  delete frmFirmEdit;
...
void __fastcall TdmMain::aDeleteFirmExecute(TObject *Sender)
{
  dmBase->FIRMS->Delete();
}
//---------------------------------------------------------------------------
...
void __fastcall TdmMain::aShowFirmsExecute(TObject *Sender)
{
  using namespace :: ntics_forms;
  ShowForm(__classid(TfrmFirms),"Фирмы",ftMDI);
}
//---------------------------------------------------------------------------
