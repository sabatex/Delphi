...
begin
  ...
  begin
    if Column.PickList.Count > 0 then
      ...
      NewImageIndex := Column.Field.AsInteger;
...
begin
  with FGrid.Datalink.Datasource.Dataset do
    Result := DataSetCompareBookmarks(FGrid.Datalink.Datasource.Dataset, Item1, Item2);
end;
...
begin
  ...
  try
    CheckBrowseMode;
    for I := FList.Count - 1 downto 0 do
      if not DatasetBookmarkValid(FGrid.DataLink.Datasource.Dataset, FList[I]) then
      begin
        Result := True;
        FList.Delete(I);
      end;
...
//      InflateRect(DHRect, 1, 1);
  ...
  begin
    Result := False;
    { TODO : Add support of IMemTableEh }
    if ViewScroll then Exit;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          FRow := FDataLink.ActiveRecord + TopDataOffset;
...
//\\
      ...
      begin // Draw contents
        ...
        begin
          ...
          begin
            DrawCellTreeArea(ACol, ARow, AState, ARect);
...
var
  NewRowCount, NewRow, NewCol: Integer;
  Field: TField;
begin
  if not FInplaceSearchingInProcess then
    StopInplaceSearch;
...
procedure TCustomDBGridEh.DrawCellTreeArea(DataCol, DataRow: Integer; AState: TGridDrawState; ARect: TRect);
var
...
finalization
  ...
  VarColCellParamsEh.Free;
  VarColCellParamsEh := nil;
