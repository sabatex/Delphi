...
{                     EhLib vX.X                        }
{                                                       }
{          TMemTableDataEh component (Build 15)         }
{                                                       }
...
{ TMemoryTreeListEh }
  ...
  private
    ...
    function GetAccountableItem(const Index: Integer): TMemoryTreeNodeEh;
  ...
  public
    ...
    function GetChildNodesForKey(StartNode: TMemoryTreeNodeEh; const KeyFieldNames: String; const ParentFieldNames: String; MemRecord: TMemoryRecordEh; ChildList: TObjectList): TMemoryTreeNodeEh;
    function GetIndexForNode(Rec: TMemoryRecordEh; ParentNode: TMemoryTreeNodeEh): Integer;
    ...
    function GetParentNodeAtKeyValue(StartNode: TMemoryTreeNodeEh; const KeyFieldNames: String; const ParentFieldNames: String; RefKeyValue: Variant): TMemoryTreeNodeEh;
    function GetParentNodeForRec(MemRecord: TMemoryRecordEh): TMemoryTreeNodeEh;
    function GetParentNodeForRefValue(RefValue: Variant): TMemoryTreeNodeEh;
    ...
    procedure GetRecordsList(List: TObjectList; Node: TMemoryTreeNodeEh; ARecurse: Boolean = True);
    procedure MoveTo(Node: TTreeNodeEh; Destination: TTreeNodeEh; Mode: TNodeAttachModeEh; ReIndex: Boolean); override;
    ...
    procedure UpdateNodeState(Node: TMemoryTreeNodeEh; IsUpdateParent: Boolean);
    property AccountableCount: Integer read GetAccountableCount;
    property AccountableItem[const Index: Integer]: TMemoryTreeNodeEh read GetAccountableItem;
    ...
    property VisibleItem[const Index: Integer]: TMemoryTreeNodeEh read GetVisibleItem; default;
...
var
  ...
  begin
    if NeedAdd then
{$IFDEF CIL}
      FFilteredRecsList.Add(TObject(Index))
{$ELSE}
...
{$ENDIF}
    ...
    begin
{$IFDEF CIL}
      FFilteredRecsList.Insert(NewIndex, TObject(Index));
{$ELSE}
...
//            FFilteredRecsList[i] := Pointer(Integer(FFilteredRecsList[i]) - 1);
  ...
  //          FFilteredRecsList[i] := Pointer(Integer(FFilteredRecsList[i]) - 1);
        ...
        for i := 0 to FFilteredRecsList.Count - 1 do
          if Integer(FFilteredRecsList[i]) >= Index then
{$IFDEF CIL}
            FFilteredRecsList[i] := TObject(Integer(FFilteredRecsList[i]) - 1);
{$ELSE}
            FFilteredRecsList[i] := Pointer(Integer(FFilteredRecsList[i]) - 1);
...
{$IFDEF CIL}
        FFilteredRecsList.Add(TObject(i));
{$ELSE}
...
begin
  if ViewAsTreeList
    ...
    else Index := Integer(FFilteredRecsList[Index]);
  MemTableData.RecordsList.DeleteRecord(Index);
end;

function TRecordsViewEh.GetRec(Index: Integer): TMemoryRecordEh;
begin
  Result := MemTableData.RecordsList[Integer(FFilteredRecsList[Index])];
end;

function TRecordsViewEh.GetValue(RecNo, ValNo: Integer): Variant;
begin
  Result := MemTableData.RecordsList.Value[Integer(FFilteredRecsList[RecNo]), ValNo];// GetValue(Integer(FFilteredRecsList[RecNo]), ValNo);
end;

procedure TRecordsViewEh.SetValue(RecNo, ValNo: Integer; const Value: Variant);
begin
  MemTableData.RecordsList.Value[Integer(FFilteredRecsList[RecNo]), ValNo] := Value;
end;
  ...
  begin
    if Index > 0
      then MemTableData.RecordsList.InsertRecord(
            MemoryTreeList.VisibleItem[Index-1].Data.Index + 1, Rec)
      else MemTableData.RecordsList.InsertRecord(0, Rec)
  end
  else
    MemTableData.RecordsList.InsertRecord(Integer(FFilteredRecsList[Index]), Rec);
end;

procedure TRecordsViewEh.SetRec(Index: Integer; const Value: TMemoryRecordEh);
begin
  MemTableData.RecordsList.Rec[Integer(FFilteredRecsList[Index])] := Value;//  SetRec(Integer(FFilteredRecsList[Index]), Value);
end;
...
begin
  MemTableData.RecordsList.RevertRecord(Integer(FFilteredRecsList[Index]));
end;
...
begin
  MemTableData.RecordsList.RefreshRecord(Integer(FFilteredRecsList[Index]), Rec);
end;
...
begin
  if ViewAsTreeList
    then Result := MemoryTreeList.VisibleItem[Index].Data
...
begin
  if ViewAsTreeList
    then Result := MemoryTreeList.AccountableItem[Index].Data
    else Result := Rec[Index];
end;
  ...
  begin
    ...
    begin
      for i := 0 to Node.Parent.Count-1 do
        if Node.Parent[i].Data.Index > Item.Index then
        begin
...
begin
  inherited Create(TMemoryTreeNodeEh);
...
begin
  FVisibleItems.Free;
  FVisibleExpandedItems.Free;
  FInsertedNodeStack.Free;
  inherited Destroy;
end;

function TMemoryTreeListEh.GetIndexForNode(Rec: TMemoryRecordEh; ParentNode: TMemoryTreeNodeEh): Integer;
var
  i: Integer;
begin
  if (ParentNode = nil) or (ParentNode.Count = 0) or (ParentNode[ParentNode.Count-1].Data.Index < Rec.Index) then
    Result := -1
  else
  begin
    Result := 0;  
    for i := ParentNode.Count-1 downto 0 do
      if ParentNode[i].Data.Index < Rec.Index then
      begin
        Result := i+1;
        Break;
      end;
...
function TMemoryTreeListEh.AddChild(const Name: string; Parent: TMemoryTreeNodeEh; MemRecord: TMemoryRecordEh): TMemoryTreeNodeEh;
var
  i: Integer;
begin
  if MemRecord.Index = MemRecord.RecordsList.Count-1 then
    Result := TMemoryTreeNodeEh(inherited AddChild(Name, Parent, MemRecord))
  else
  begin
    Result := nil;
    if Parent = nil then
      Parent := TMemoryTreeNodeEh(Root);
    if (Parent.Count = 0) or (Parent = nil) or (Parent[Parent.Count-1].Data.Index < MemRecord.Index) then
      Result := TMemoryTreeNodeEh(inherited AddChild(Name, Parent, MemRecord))
    else
      for i := Parent.Count-1 downto 0 do
        if Parent[i].Data.Index < MemRecord.Index then
        begin
          Result := TMemoryTreeNodeEh(inherited AddChild(Name, Parent, MemRecord));
...
function TMemoryTreeListEh.GetNode(StartNode: TMemoryTreeNodeEh; MemRecord: TMemoryRecordEh): TMemoryTreeNodeEh;
begin
  Result := TMemoryTreeNodeEh(inherited GetNode(StartNode, MemRecord));
end;

function TMemoryTreeListEh.GetVisibleItem(const Index: Integer): TMemoryTreeNodeEh;
begin
  ...
  Result := TMemoryTreeNodeEh(FVisibleExpandedItems.Items[Index]);
end;

function TMemoryTreeListEh.GetAccountableItem(const Index: Integer): TMemoryTreeNodeEh;
begin
  if FVisibleItemsObsolete then
    BuildVisibleItems;
  ...
  Result := TMemoryTreeNodeEh(FVisibleItems.Items[Index]);
end;
...
function TMemoryTreeListEh.GetNodeAtValue(StartNode: TTreeNodeEh; const FieldNames: String; const Value: Variant): TTreeNodeEh;
var
  I: Integer;
  CurNode: TTreeNodeEh;
...
function TMemoryTreeListEh.GetParentNodeAtKeyValue(StartNode: TMemoryTreeNodeEh;
  const KeyFieldNames: String; const ParentFieldNames: String;
  RefKeyValue: Variant): TMemoryTreeNodeEh;
var
  I: Integer;
  CurNode: TMemoryTreeNodeEh;
  CurMemRec: TMemoryRecordEh;
begin
  Result := TMemoryTreeNodeEh(Root);
  if StartNode = nil then
    StartNode := TMemoryTreeNodeEh(Root);
  for I := 0 to StartNode.Count - 1 do
  begin
    CurNode := StartNode.Items[I];
    CurMemRec := TMemoryRecordEh(CurNode.Data);
    ...
    begin
      ...
      if Result <> TMemoryTreeNodeEh(Root) then
        Break;
    end;
  end;
end;

function TMemoryTreeListEh.GetParentNodeAtKey(StartNode: TMemoryTreeNodeEh;
 const KeyFieldNames: String; const ParentFieldNames: String;
 MemRecord: TMemoryRecordEh): TMemoryTreeNodeEh;
var
  I, Index: Integer;
  CurNode: TMemoryTreeNodeEh;
  CurMemRec: TMemoryRecordEh;
...
function TMemoryTreeListEh.GetParentNodeForRec(MemRecord: TMemoryRecordEh): TMemoryTreeNodeEh;
var
  Index: Integer;
  MTIndex: TMTIndexEh;
begin
...
function TMemoryTreeListEh.GetChildNodesForKey(StartNode: TMemoryTreeNodeEh;
  const KeyFieldNames: String; const ParentFieldNames: String;
  MemRecord: TMemoryRecordEh; ChildList: TObjectList): TMemoryTreeNodeEh;
var
  I: Integer;
  CurNode: TMemoryTreeNodeEh;
  CurMemRec: TMemoryRecordEh;
...
begin
  if TMemoryTreeNodeEh(Node1).Data.Index > TMemoryTreeNodeEh(Node2).Data.Index then
    Result := 1
  else if TMemoryTreeNodeEh(Node1).Data.Index < TMemoryTreeNodeEh(Node2).Data.Index then
    Result := -1
  else
    Result := 0;
end;
...
function TMemoryTreeListEh.AddChildAtKey(const Name, KeyFieldNames , ParentFieldNames: String;
  MemRecord: TMemoryRecordEh): TMemoryTreeNodeEh;
var
  ParentNode: TMemoryTreeNodeEh;
  ChildNode, LastParent: TMemoryTreeNodeEh;
  Index, i: Integer;
  MTIndex: TMTIndexEh;
...
begin
  ...
  begin
    LastParent := TMemoryTreeNodeEh(FInsertedNodeStack[FInsertedNodeStack.Count-2]);
//    if VarEquals(LastParent.Data.DataValues[KeyFieldNames, dvvValueEh],
//         MemRecord.DataValues[ParentFieldNames, dvvValueEh]) then
    if VarEquals(LastParent.Data.DataIndexValues[FRecordsViewEh.TreeViewKeyFields, dvvValueEh],
         MemRecord.DataIndexValues[FRecordsViewEh.TreeViewRefParentFields, dvvValueEh]) then
    begin
    ...
    begin
      LastParent := TMemoryTreeNodeEh(FInsertedNodeStack[FInsertedNodeStack.Count-1]);
//      if VarEquals(LastParent.Data.DataValues[KeyFieldNames, dvvValueEh],
//           MemRecord.DataValues[ParentFieldNames, dvvValueEh])
      if VarEquals(LastParent.Data.DataIndexValues[FRecordsViewEh.TreeViewKeyFields, dvvValueEh],
...
//    else KeyValue := ParentNode.Data.DataValues[KeyFieldNames, dvvValueEh];
    else KeyValue := ParentNode.Data.DataIndexValues[FRecordsViewEh.TreeViewKeyFields, dvvValueEh];
  Result := TMemoryTreeNodeEh(AddChild(Name, ParentNode, MemRecord));
  ...
  //    if Index.FindRecordIndexByKey(MemRecord.DataValues[KeyFieldNames, dvvValueEh], RecIndex) then
    ...
    for i := 0 to ChildList.Count-1 do
      MoveTo(TMemoryTreeNodeEh(ChildList[i]), Result, naAddChildEh, True);
    ChildList.Free;
  end;
  if IsInsertInStack
...
function TMemoryTreeListEh.UpdateParent(Node: TMemoryTreeNodeEh; const KeyFieldNames: String;
  const ParentFieldNames: String; MemRecord: TMemoryRecordEh; ReIndex: Boolean): TMemoryTreeNodeEh;
var
  Index: Integer;
begin
  Result := GetParentNodeAtKey(nil, KeyFieldNames, ParentFieldNames, MemRecord);
  ...
  begin
    Index := GetIndexForNode(Node.Data, Result);
    if Index >= 0
...
procedure TMemoryTreeListEh.UpdateNodeState(Node: TMemoryTreeNodeEh; IsUpdateParent: Boolean);
begin
  //                                                    //-1???????????
  FVisibleItemsObsolete := True;  // Optimize
  if Node.VisibleCount > 0
    then Node.Visible := True
    else Node.Visible := FRecordsViewEh.FilterRecord(Node.Data, -1);
  if IsUpdateParent and (Node.Parent <> nil) and
    (Node.Parent <> FRoot) and (Node.Visible <> Node.Parent.Visible)
    {(Node.Parent.VisibleCount = 0)}
  then
    UpdateNodeState(Node.Parent, True);
end;

procedure TMemoryTreeListEh.UpdateNodesState(Parent: TMemoryTreeNodeEh);
var
  i: Integer;
begin
  if Parent = nil then
...
procedure TMemoryTreeListEh.GetRecordsList(List: TObjectList; Node: TMemoryTreeNodeEh;
...
var
  Parent, ANode: TMemoryTreeNodeEh;
...
function TMemoryTreeListEh.GetParentNodeForRefValue(RefValue: Variant): TMemoryTreeNodeEh;
var
  Index: Integer;
  MTIndex: TMTIndexEh;
begin
...
procedure TMemoryTreeListEh.MoveTo(Node, Destination: TTreeNodeEh;
...
destructor TMemoryTreeNodeEh.Destroy;
begin
  Owner.FVisibleItemsObsolete := True;
  if Owner.FInsertedNodeStack.IndexOf(Self) >= 0 then
    Owner.FInsertedNodeStack.Clear;
//  if Owner.FLastParentNodeForNewChild = Self then
//    Owner.FLastParentNodeForNewChild := nil;
  inherited Destroy;
end;

function TMemoryTreeNodeEh.GetData: TMemoryRecordEh;
begin
  Result := TMemoryRecordEh(inherited Data);
end;

function TMemoryTreeNodeEh.GetItem(const Index: Integer): TMemoryTreeNodeEh;
begin
  Result := TMemoryTreeNodeEh(inherited Items[Index]);
end;

function TMemoryTreeNodeEh.GetOwner: TMemoryTreeListEh;
begin
  Result := TMemoryTreeListEh(inherited Owner);
end;

function TMemoryTreeNodeEh.GetParent: TMemoryTreeNodeEh;
begin
  Result := TMemoryTreeNodeEh(inherited Parent);
end;

procedure TMemoryTreeNodeEh.SetParent(const Value: TMemoryTreeNodeEh);
begin
  inherited Parent := Value;
end;

procedure TMemoryTreeNodeEh.SortByFields(const SortByStr: string);
var
  AParamSort: TObject;
begin
  if Assigned(Owner.FRecordsViewEh.OnParseOrderByStr) then
  begin
    AParamSort := Owner.FRecordsViewEh.OnParseOrderByStr(SortByStr);
    try
      SortData(Owner.FRecordsViewEh.OnCompareTreeNode, AParamSort, False);
    finally
      AParamSort.Free;
    end;
  end;
