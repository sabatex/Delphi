...
Known Issues:
-----------------------------------------------------------------------------}
// $Id: JvQEmbeddedForms.pas,v 1.1 2004/12/04 00:26:03 asnepvangers Exp $

unit JvQEmbeddedForms;
...
uses 
  Qt, 
  QWindows, QForms, Classes, JvQExControls,
  JvQComponent;
...
type
  ...
  TJvEmbeddedFormPanel = class(TJvExFrameControl)
  private
...
begin 
  ...
  FBitmap.Handle := QPixmap_create;
  try
    FLinkedForm.ControlState := FLinkedForm.ControlState + [csPaintCopy];
    QPixmap_grabWidget(FBitmap.Handle, FLinkedForm.Handle, 0, 0, FLinkedForm.Width, FLinkedForm.Height);
  finally
    FLinkedForm.ControlState := FLinkedForm.ControlState - [csPaintCopy];
  end;  
  try
...
begin
  if (FLinkedForm <> nil) and (FLinkedForm.Parent = Self) then
    ...
    begin
      ...
      Parent := nil;
      Pt := Point(Left, Top);
      QWidget_reparent(Handle, nil, @Pt, false);
      // IMPORTANT!!! Don't set BorderStyle unless Parent = nil!!!
      BorderStyle := ABorderStyle;
      Position := APosition;
      AutoScroll := B;
      THackedForm(FLinkedForm).ReSubmitFlags;
      Show;
    end;
end;
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$RCSfile: JvQEmbeddedForms.pas,v $';
    Revision: '$Revision: 1.1 $';
    Date: '$Date: 2004/12/04 00:26:03 $';
    LogPath: 'JVCL\run'
  );
