...
Known Issues:
-----------------------------------------------------------------------------}
// $Id: JvgSysRequirements.pas,v 1.23 2005/10/28 08:37:23 marquardt Exp $
...
type
  ...
  published
    ...
    property WindowsVersions: TWindowsVersionSet read FWindowsVersions write FWindowsVersions default AllWindowsVersions;
    ...
    begin
      ...
      if DoShowWarning then
        Application.MessageBox(PChar(ReportMessage),
          PChar(ExtractFileName(ParamStr(0))), MB_OK + Icon);
      if DoHalt then
...
begin
  ...
  TestMinMax(BitsPerPixel, MinColorDepth, MaxColorDepth, RsMinColorDepthReq, RsMaxColorDepthReq, RsBetweenColorDepthReq);
  ...
  TestMinMax(Refresh, MinVideoRefreshRate, MaxVideoRefreshRate, RsMinRefreshReq, RsMaxRefreshReq, RsBetweenRefreshReq);
