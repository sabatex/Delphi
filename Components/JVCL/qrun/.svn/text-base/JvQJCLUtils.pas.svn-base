...
-----------------------------------------------------------------------------}
// $Id: JvQJCLUtils.pas,v 1.46 2005/11/01 20:47:11 asnepvangers Exp $
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$RCSfile: JvQJCLUtils.pas,v $';
...
begin
  Result := QWindows.IsCharAlpha(Char(Ch));
end;
...
begin 
  ...
  for I := 1 to Length(Str) do
    if Str[I] in ['0'..'9', '-', '+', AnsiChar(DecimalSeparator)] then 
      LStr := LStr + Str[I]; 
  try 
...
begin 
  for I := 1 to Length(Str) do
    if Str[I] in ['0'..'9', '-', '+', 'e', 'E', AnsiChar(DecimalSeparator)] then 
      LStr := LStr + Str[I]; 
  Result := Def; 
  if LStr <> '' then
  try
...
begin
  ...
  for I := 1 to Length(Result) do
    if Result[I] in ['0'..'9', '-', '+',
        AnsiChar(DecimalSeparator), AnsiChar(ThousandSeparator)] then
    begin
      Result[J] := Result[I];
      Inc(J);
    end;
